...
**Last updated:** 2026-05-15, after 3.1.8 planning round close (Option α + #54 denominator correction locked; ranking-gap fix deferred to 3.2). **Phase 1 CLOSED. Phase 2 CLOSED. Phase 3 IN PROGRESS — sub-bundles 3.1, 3.1.5, 3.1.6, 3.1.7 closed; 3.1.8 planning round closed; next: 3.1.8 mech.1 (relaxedMatchAtK denominator switch to K-based).**
...
**Eval threshold for post-eval-pass flip commit (R3 lock):** `aggregateScore 0.0833` is the mech.6 baseline. Lowering this threshold for sub-bundle 3.2+ requires a HANDOFF amendment with rationale. Raising it is preferred — sub-bundle 3.2 (order ingest + sales velocity) and any expectedHandles curation should both pull this number up. *[Retired in 3.1.7 close — see "R3 retirement and R3.0/R3.1/R3.2 quality ladder" subsection below. R3=0.0833 was tautological (set TO the empirical baseline, never at risk of regressing); replaced by R3.0=0.2917 anchored against post-3.1.7 actual. Original framing preserved here for historical context.]*
...
- Threshold remains locked at the R3 value of 0.0833. Threshold revision is a separate decision at the post-eval-pass flip commit (or whenever a sub-bundle moves the number up). *[Retired in 3.1.7 close — see "R3 retirement and R3.0/R3.1/R3.2 quality ladder" subsection in the 3.1.7 close section below.]*
...
- Diagnostic only — does NOT modify the locked R3 threshold (0.0833). *[Retired in 3.1.7 close — see "R3 retirement and R3.0/R3.1/R3.2 quality ladder" subsection in the 3.1.7 close section below.]*
...
47. Bulk-mutation scripts vs Railway proxy stability: mech.3's live pass dropped on a transient P1017 (Server has closed the connection) error after batch 1 of 7 (50/312 rows flipped). The script's PENDING_REVIEW-status idempotency contract made resume safe — re-invoking with the same args produced the intended final state in a second wave (262 rows). Acceptable for the script's current "shop onboarding kit" use case, but if Phase 5 multi-shop onboarding scales bulk-approves to N merchants, building automatic-resume-on-disconnect into the script (or moving the work into a queue-driven worker) becomes load-bearing. Captured here as a pattern recognition; out of 3.1.7 scope.
48. `fashion-oos-stress-2` fixture (category=saree) is architecturally unaddressable on the dev shop catalog: zero products with saree/sari titles or productType per `.pr-3-1-7-mech-4-artifacts/_inspect-saree-shorts-output.txt`. Both vocabulary and rule-engine paths have no work — neither can produce category=saree ProductTag rows when no products match the value. The fixture's design presumably reflects future merchant use cases (Indian-ethnic boutiques) not present in this dev shop. When a real merchant with saree merchandise onboards, the existing `saree` vocab entry in `AXIS_OPTIONS.FASHION.category.values` + a saree-targeting rule (or AI-tagger pass) will close the gap automatically. Out of 3.1.7 scope.
...
50. 3.1.7's 5-mech chain (variant-filter relocation, variant-loading wire, secondary-axis bulk approve, category=shorts rule, R3 retirement) was developed, calibrated, and verified entirely against a single FASHION-mode dev shop (`ai-fashion-store.myshopify.com`). The architecture (`AXIS_OPTIONS`, `SEED_RULES`, `HARD_FILTER_AXES`, `STARTER_AXES`, the shop-onboarding-kit scripts `bulk-approve-tags.ts` / `bulk-reembed-products.ts` / `apply-rules-to-shop.ts`) is multi-tenant by design, parameterized per store mode and per shop. But every concrete calibration — mech.3a's 0.8 confidence threshold, mech.5's R3 retirement against the post-mech.4 anchor, the v2 pipeline's relaxedMatchAtK fixture-driven scoring, the Stage 6 binary `available` decision (op debt #42 / #45), the rule-engine title-pattern matching approach (mech.4), the per-axis distinct-product coverage diagnostics — derives from fashion-shop evidence. Non-fashion merchants (ELECTRONICS, FURNITURE, BEAUTY, JEWELLERY, GENERAL) may exhibit different confidence distributions, different fixture intent patterns, different ranking-vs-availability tradeoffs, different rule-pattern viability (electronics products often have model-number titles rather than descriptive titles, etc.). **Multi-tenant verification needs its own planning round** when a non-fashion merchant is ready to onboard. Out of 3.1.7/3.1.8 scope. Phase 5 multi-shop onboarding territory.
51. Dev shop's TaggingRule table contains rules that diverge from `SEED_RULES.FASHION`. Discovered at mech.4.5 (`.pr-3-1-7-mech-4-artifacts/_inspect-rule-state-output.txt`): pre-mech.4 the shop had 6 rules, two of which (`Kurta → category=kurta` priority 102, `Jeans → category=jeans` priority 103) have category effects that don't appear in `SEED_RULES.FASHION`'s codebase definition. Two more (`Linen mention → sub_category=linen` priority 104, `Cotton mention → sub_category=cotton` priority 105) write to `sub_category` axis whereas the equivalent SEED_RULES entries write to `material` axis. The dev shop was apparently hand-customized at some point. **Consequence:** `seedRules()` would write a different rule set than what's actually deployed; future shops onboarded post-3.1.7 will diverge from the dev shop's behavior. **For multi-tenant verification (op debt #50) this is a load-bearing reconciliation** — when does a shop's TaggingRule become out-of-sync with `SEED_RULES`, and what's the canonical-source contract? Cleanup options: (a) update `SEED_RULES.FASHION` to match the dev shop's actual rules (treats deployed state as canonical), (b) reset the dev shop's rules to match SEED_RULES (treats code as canonical, loses any hand-customizations), (c) document divergence as an expected merchant-customization feature (treats SEED_RULES as defaults, not canonical). **3.1.8 planning round close: direction NAMED as option (c) — "SEED_RULES is the starter template, not canonical for any deployed shop"; implementation deferred to 3.2 as part of the catalog-data cluster.** **mech.4's eval-regression finding (broader category coverage hurt the kurta fixture) was a downstream effect of this discrepancy** — the unexpected kurta-rule activation diluted top-6 ranking. Re-framed at 3.1.8 close (op debt #54) as eval-invisible-not-regression under K-based normalization. Out of 3.1.7 scope.
52. Stage 5's `stage5Diversity` iterates Stage 4 input in similarity-distance order; the `rerankBoosts` attached by Stage 3 feed Stage 6's finalScore only, not Stage 5's top-K selection. Existing FASHION rerankers (occasion, fit, color, body_type) influence display ordering but don't affect which candidates make top-K. Identified during 3.1.8 planning round Thread 3 (artifacts 10/15) when fix (a) — Stage 3 rerank tag-overlap signal — produced NULL eval impact (artifact 14). Future Stage 3 rerank work must be paired with a Stage 5 change (e.g., the d.4 "rerank-aware iteration order" shape from Thread 3 artifact 18) for the rerank to be selection-effective. **Priority: Medium.** Triage verdict (Thread 2 artifact 26): (N) carry to 3.2 or (P) Phase 5+ depending on whether 3.2 picks d.2-strict alone or a multi-stage architectural rework.
53. Stage 2's `candidatePoolSize=50` caps narrowing dilution: when Stage 1's pool size exceeds 50, Stage 2's narrowing can drop tag-complete candidates that don't rank in the top-50 by pure semantic similarity. The 3 STAGE_2_NARROWING_DROPS_SATISFYING fixtures (`fashion-oversized-fit-kurta`, `fashion-going-out-outfit`, `fashion-wedding-reception`) all hit this bottleneck (Thread 3 artifact 13). Possible mitigations: raise pool size, weight Stage 2 by tag-completeness (d.3 shape), or move the filter upstream (d.2-strict). **Priority: High.** Triage verdict (Thread 2 artifact 27): (N) carry to 3.2 as the d.2-strict primary fix site.
54. **CLOSED in 3.1.8-mech.1.** `relaxedMatchAtK` (in `app/lib/recommendations/v2/eval/scoring.ts:91`) normalized by `Math.max(1, top.length)` rather than by K, creating a small-pool incentive (fixtures with fewer returned candidates were structurally favored). Identified during 3.1.8 planning round Thread 3 (artifact 20 § 7); Thread 2 (artifact 28) confirmed retroactively that the 3.1.7 mech.4 kurta regression (0.3333 → 0.2917) was an artifact of this property, not a real ranking regression. Closed in 3.1.8-mech.1 via switch to `Math.max(1, k)`. R3 ladder re-anchored: R3.0 = 0.2778 (K-based), R3.1 ≥ 0.2778, R3.2 ≥ 0.50. **Note: pre-3.1.8 R3 values (0.0833, 0.1667, 0.3333, 0.2917) are non-comparable to post-3.1.8 numbers due to denominator semantics change.**
55. Deferred architectural finding (3.1.8 Option α): the ranking-vs-tag-completeness gap surfaced by 3.1.7 mech.4 carries to 3.2 with a recommended primary fix shape (d.2-strict — Stage 1 secondary-axis hard filter on extracted query axes; Thread 3 artifact 18). d.2-strict's empty-Stage-1 risk on real merchant queries must be measured against production traffic post-3.1.8-flip before d.2-strict parameterization is locked. **Priority: High.** Triage verdict (Thread 2 artifact 29): (N) carry to 3.2 by definition under Option α.
...
**Mech.3a + Mech.3 status:** mech.3 paused mid-flight at a decision gate after the dry-run revealed 22.8% of 404 PENDING_REVIEW candidates were sub-0.8 AI-tagger confidence (size_range outlier at 75% sub-0.8). mech.3a (`ef9a61f`) extended `scripts/bulk-approve-tags.ts` with `--min-confidence=<float>` flag (default 0; preserves backward-compat with 3.1 mech.6's (gender, category) baseline). mech.3 (`<this commit's SHA>`) resumed with `--min-confidence=0.8` and landed in two waves on the dev shop: **wave 1** (`.pr-3-1-7-mech-3-artifacts/03-bulk-approve-live.txt`) flipped 50/312 then dropped on a transient Prisma `P1017: Server has closed the connection` error after batch 1 (script exited code 1, no further batches attempted); **partial-state inspection** (`03a-partial-state-inspection.txt`) confirmed 50 APPROVED via the `actorId='system://3.1.7-mech.3-bulk-approve'` audit trail, 262 high-confidence PENDING_REVIEW remaining; **wave 2** (`03b-bulk-approve-live-resumed.txt`) re-invoked the script with the same args — the script's `status: 'PENDING_REVIEW'` candidate-filter idempotency contract made resume safe — and flipped all 262 remaining rows in one contiguous batch sequence. End state: **312 high-confidence tags APPROVED across the seven secondary axes** (occasion 66 + color_family 48 + material 51 + fit 39 + season 48 + size_range 15 + style_type 45 = 312 ✓), eligibility intersection (products with APPROVED on all 7 axes) = 2. The 92 sub-0.8 candidates remain PENDING_REVIEW for future merchant-review-portal work. Mech.3 D1 traded eval-lift coverage (~22% fewer approvals than the unfiltered alternative) for diagnostic clarity (mech.3.5's eval lift will be a clean signal, no false-positive bias from low-confidence tag matches). Eval re-run + per-axis catalog coverage analysis ship in mech.3.5; mech.3 captures the 12-file artifact set in `.pr-3-1-7-mech-3-artifacts/` (see `00-readme.md` for the chain narrative).

**Mech.3.5 status:** mech.3.5 (`<this commit's SHA>`) verifies mech.3's bulk-approve via three artifacts: Stage 1 regression-clean (universe stays at 1,169, per-fixture distribution identical to mech.2.5), per-axis catalog coverage scan across all 9 axes (the 312 high-confidence flips visible across the 7 secondary axes; matches `04-post-pass-axis-coverage.txt` exactly; surfaces a `size_range` distinct-product-coverage diagnostic of just 3 products carrying the 15 APPROVED size_range tag rows), and eval re-run (EvalRun `cmp03wlih0000q7x49weys2bu`) showing **aggregateScore moved 0.1667 → 0.3333** (the first non-degenerate baseline movement of the 5-mech chain; +1 PASS `fashion-casual-office-shirts`, +1 PARTIAL `fashion-oversized-fit-kurta`, +3 fixtures gaining partial-credit relaxed=0.17 from 1/6-card matches; full attribution sums to the +0.1666 drift exactly). Final state: **3 PASS / 1 PARTIAL / 8 FAIL**. The remaining 8 FAILs decompose as: 3 with relaxed=0.17 partial credit (linen-shirts-white, minimalist-daily-wear, oos-stress-1) where 1/6 cards matches all axes; 2 no-hard-filter fixtures (going-out-outfit, wedding-reception) where Stage 5's diversity quotas don't surface APPROVED-occasion candidates from the 1000-product universe; and 3 blocked by axes outside mech.3's scope (festive-kurta-women per op debt #43 gender=female, oos-stress-2/summer-shorts-size-m per mech.4 conditional vocabulary gap). **mech.4 conditional gate is settled NOT cuttable** — the saree/shorts/gender=female fixtures stay at 0 candidates without further work.

**Mech.4 status:** mech.4 (`<this commit's SHA>`) ships `category=shorts` via rule-engine seed addition (`rule-seeds.ts` SEED_RULES.FASHION canonical addition, plus one-off DB insert into the dev shop's existing TaggingRule rows since `seedRules()` is shop-level idempotent — dev shop had 7 seeded FASHION rules from earlier onboarding). New script `scripts/apply-rules-to-shop.ts` joins `bulk-approve-tags.ts` and `bulk-reembed-products.ts` as the third item in the "shop onboarding kit": shop-parameterized, idempotent, dry-runnable, forensic via `--actor-id`. Dev-shop live run produced **253 RULE-source APPROVED `category=shorts` rows** (audit trail via `actorId='system://3.1.7-mech.4-apply-rules'`, dry-run count == live-run count == 253, 1:1 product-touched/tags-written ratio since category is single-value). Catalog category coverage jumped 50 → 303 distinct APPROVED products (~26% of the 1,168-product catalog, up from ~4.3%). The 253 count exceeded the prompt's "~20-50" prediction because `title_contains "shorts"` catches co-ord sets ("T-Shirt & Shorts Co-Ord Set" etc.) in addition to strict shorts-typed products — consistent with the existing rule-seeds.ts pattern (`"Linen material"` uses `title_contains "linen"` similarly broadly); accepted as a legitimate categorical signal for shorts-relevant chat queries. **Saree deliberately skipped:** pre-flight inspection (`.pr-3-1-7-mech-4-artifacts/_inspect-saree-shorts-output.txt`) found zero saree-ish products on the dev shop, so vocabulary expansion for `category=saree` has no work to perform. `fashion-oos-stress-2` stays FAIL on this dev shop catalog. Pre-flight also surfaced broader category-coverage gap: only 51/1,168 products had any category tag pre-mech.4 — captured as op debt #49. Op debts #48 (saree fixture unaddressable here) and #49 (broader category-coverage gap) logged. Eval re-run ships in mech.4.5.

**Mech.4.5 status:** mech.4.5 (`<this commit's SHA>`) verifies mech.4 and surfaces a framing correction. **What actually happened in mech.4's apply-rules-live pass:** the 253 RULE-source writes decompose as **200 `category=kurta` + 53 `category=shorts`**, not 253 pure shorts as mech.4's commit message implied. Cause: the dev shop's TaggingRule table contained two pre-existing rules with category effects (`Kurta → category=kurta` at priority 102, `Jeans → category=jeans` at priority 103) that had never been applied to the catalog; mech.4's `--axes=category` invocation fired them retroactively alongside the new shorts rule. Captured via DB inspection at mech.4.5 (`.pr-3-1-7-mech-4-artifacts/_inspect-rule-state-output.txt`) — audit trail confirms 200 newValue=kurta + 53 newValue=shorts, all `actorId='system://3.1.7-mech.4-apply-rules'`. The mech.4 SHA citation stands (it did write 253 RULE rows; the framing was wrong, not the count). **Eval result: aggregateScore 0.3333 → 0.2917 (REGRESSION −0.0417, EvalRun `cmp1dlvil0000q7gg3scb6rt7`).** Cause: `fashion-oversized-fit-kurta` regressed PARTIAL (0.50) → FAIL (0). Pre-mech.4 the kurta fixture's Stage 1 returned 2 candidates, 1 of which had both `category=kurta` AND `fit` APPROVED → relaxedMatchAtK=0.50. Post-mech.4 Stage 1 returns 202 candidates, but only 2 still have `fit` APPROVED (catalog-wide fit coverage is just 39 distinct products); Stage 5's diversity quotas drop those 2 from top-6 in favor of higher-similarity kurta products that lack `fit` APPROVED → relaxedMatchAtK=0. **The finding: broader category coverage HURT eval when secondary-axis coverage didn't scale proportionally.** Load-bearing evidence for op debts #45, #46, #49. Cumulative 3.1.7 chain: 0.0833 → 0.2917 (+0.2084, 3.5× lift) across 4 mechs with mech.4 contributing the lone regression. **Final 3.1.7 baseline = 0.2917** — the anchor mech.5's R3 retirement cites. **Op debts #50 (multi-tenant verification deferred) and #51 (pre-existing-rules-not-in-SEED_RULES discrepancy) logged.**

### Sub-bundle 3.1.7 close: 5-mech chain (universe correction + variant-loading + secondary-axis approval + category=shorts rule + R3 retirement); eval baseline 0.0833 → 0.2917 (3.5× cumulative)

Sub-bundle 3.1.7 closes here. Mech chain: planning round (`6176311`) → mech.1 (`bbc79d1`) → mech.1.5 (`89d42d7`) → mech.2 (`b9a5152`) → mech.2.5 (`520e60c`) → mech.3a (`ef9a61f`) → mech.3 (`7aa2ea2`) → mech.3.5 (`c17eae1`) → mech.4 (`688019d`) → mech.4.5 (`b7cb07c`) → this close. **Final 3.1.7 eval baseline: aggregateScore = 0.2917** across 12 fixtures (3 PASS / 0 PARTIAL / 9 FAIL; EvalRun `cmp1dlvil0000q7gg3scb6rt7`). Cumulative lift: **+0.2084 over pre-3.1.7 baseline (0.0833 → 0.2917, 3.5×)** across the 4 mechs that touched the catalog (mech.1 universe expansion, mech.2 variant-loading wire, mech.3 secondary-axis bulk approve, mech.4 retroactive rule-engine pass). **Mech.3 was the load-bearing improvement** (+0.1666, 1 fixture FAIL→PASS + 1 fixture FAIL→PARTIAL + 3 fixtures gaining partial-credit relaxed=0.17). Mech.4 surfaced architectural texture worth carrying into 3.1.8 planning (see "Architectural finding" subsection below); the +0.2084 cumulative captures both the mech.3 lift and the mech.4 regression honestly.

**What 3.1.7 actually shipped:**

- ✅ **Variant-filter relocation** (PR-3.1.7-mech.1, `bbc79d1`): `availableForSale=true EXISTS` predicate moved from Stage 1 hard-filter to Stage 6 binary attachment per mech.1 D1; Stage 1 input universe expanded from 29 → 1,169 products. Closes op debt #11 architecturally (OOS-substitute concern resolves via "Stage 6 just doesn't surface OOS items" rather than a deferred Stage 5 substitute pass).
- ✅ **Variant-loading wire** (PR-3.1.7-mech.2, `b9a5152`): new Stage 5.5 in `pipeline.server.ts` orchestrator (`loadAndAttachVariants` mirrors `loadAndAttachTags`'s shape; variant selection rule mirrors v1 `findSimilarProducts`: `orderBy: [{ availableForSale: "desc" }, { price: "asc" }], take: 1`). Stage 6's `formatProductCard` now sources `variantId`, `compareAtPrice`, `available`, `price` from `c.loadedVariant`. mech.1 D1's `bool_or` aggregate retired in the same commit. Closes op debt #15.
- ✅ **Secondary-axis bulk approve** (PR-3.1.7-mech.3, `7aa2ea2`; +PR-3.1.7-mech.3a `ef9a61f` extending `scripts/bulk-approve-tags.ts` with `--min-confidence=<float>` flag, default 0): 312 high-confidence (≥0.8) flips across occasion / color_family / material / fit / season / size_range / style_type. 92 sub-0.8 candidates remain PENDING_REVIEW for future merchant-review-portal work. Two-wave landing (50 + 262) due to a transient Prisma P1017 connection drop after batch 1; the script's `status: 'PENDING_REVIEW'` candidate-filter idempotency contract made resume safe (op debt #47).
- ✅ **Category=shorts rule-engine seed** (PR-3.1.7-mech.4, `688019d`): new rule in `rule-seeds.ts` SEED_RULES.FASHION canonical + one-off DB insert into the dev shop's existing TaggingRule rows. New `scripts/apply-rules-to-shop.ts` joins `bulk-approve-tags.ts` and `bulk-reembed-products.ts` as the third item in the "shop onboarding kit". Live-pass produced 253 RULE-source APPROVED rows; per mech.4.5 framing correction the rows decompose as 200 `category=kurta` (from pre-existing rule fired retroactively, op debt #51) + 53 `category=shorts` (the new rule).
- ✅ **R3 retirement** (this commit): R3=0.0833 retired explicitly; replaced by R3.0/R3.1/R3.2 quality ladder anchored against post-3.1.7 actual. See subsection below.
- ⏭️ **Saree fixture deliberately skipped** (op debt #48): zero saree-ish products on the dev shop catalog. Vocabulary expansion for `category=saree` has no work to perform. `fashion-oos-stress-2` stays FAIL until a real merchant with saree merchandise onboards.
- ⏭️ **Flip itself deferred to 3.1.8** per planning round Section 6 line 156.

**Four verification commits** captured the per-mech diagnostic separability: mech.1.5 (`89d42d7`) for universe-expansion attribution, mech.2.5 (`520e60c`) for variant-loading wire verification (probe shows 100% populated variantId on all returned ProductCards), mech.3.5 (`c17eae1`) for secondary-axis-coverage attribution (the first non-degenerate eval movement of the chain: 0.1667 → 0.3333), mech.4.5 (`b7cb07c`) for the rule-engine pass + framing correction + the architectural finding.

#### R3 retirement and R3.0/R3.1/R3.2 quality ladder

R3=0.0833 retired explicitly. It was tautological from inception: the 0.0833 value was the empirical pre-3.1.7 baseline (1 PASS at 1.0 / 12 fixtures = 0.0833), set TO the value it was meant to defend, meaning nothing had ever been at risk of regressing below it. R3 carried no diagnostic information; passing it was guaranteed by definition.

Re-anchored quality ladder tied to fixture pass-rate, mirroring the planning artifact's Section 5 line 122 suggestion:

- **R3.0 (post-3.1.7 actual):** aggregateScore = 0.2917 across 12 fixtures, derived against universe-corrected catalog (1,169-product Stage 1 universe, 29 with available variants), secondary-axis APPROVED on the 50-product calibration sample (312 high-confidence flips across 7 axes), and rule-engine category coverage (303/1,168 = 26% products with APPROVED category, up from 4.3% pre-mech.4). EvalRun `cmp1dlvil0000q7gg3scb6rt7`. 3 PASS / 0 PARTIAL / 9 FAIL.
- **R3.1 (3.1.8 flip target):** ≥ 0.2917 (no regression). 3.1.8 ships the v1→v2 flip (per planning Section 6 line 156); eval must not move below R3.0 at flip time. Note: the mech.3.5 peak of 0.3333 is documented as a momentary state pre-mech.4, NOT an R3.1 target; recovering 0.3333+ requires Stage 3 rerank or Stage 5 quota policy changes, which are architectural backlog beyond 3.1.8's scope (see "Architectural finding" below).
- **R3.2 (Phase 5 multi-mode):** ≥ 0.50. Substantive cross-fixture pass-rate target. Implies broader catalog coverage, more rule-engine seeds, possibly Stage 3 / Stage 5 ranking changes, and multi-tenant verification (op debt #50).

R3 retirement is documentation-only (the threshold has no code surface — confirmed via `.pr-3-1-7-planning-artifacts/17-r3-threshold-references.txt`). Earlier HANDOFF passages referencing R3 as a live threshold have been marked with historical-context footnotes pointing back to this subsection (three references at the planning-round close, 3.1.5 close, and 3.1.6 close subsections).

#### Architectural finding: ranking-vs-tag-completeness gap (3.1.8 planning input)

mech.4's retroactive rule-engine pass added 200 RULE-source `category=kurta` rows (from a pre-existing rule that fired retroactively — see op debt #51) alongside the new 53 `category=shorts` rows. The unexpected kurta-rule activation lifted the `fashion-oversized-fit-kurta` fixture's Stage 1 candidate pool from 2 to 202 products. But only 2 of those 202 carried APPROVED `fit` tags (catalog-wide fit coverage is just 39 distinct products). Stage 2 semantic similarity + Stage 5 diversity quotas selected 6 cards from the 202; none of the 6 had `fit` APPROVED. The fixture regressed PARTIAL (0.50) → FAIL (0). aggregateScore dropped from 0.3333 to 0.2917.

The finding: **Stages 2-5's ranking signals don't favor "products whose APPROVED tag-set matches the query's extracted attributes."** Stage 3 reranks on similarity / occasion / fit / color via embeddings (semantic similarity), not via tag-set overlap with the query's intent. Stage 5 selects top-K via diversity quotas (CATEGORY_MAX=2, COLOR_FAMILY_MAX=3), not via per-card filter satisfaction likelihood. Broader coverage on one axis can therefore HURT eval scores when secondary-axis coverage doesn't scale proportionally — the most-tag-complete candidates get displaced by higher-similarity candidates that lack the relevant secondary tags.

This is the convergent architectural surface implied by op debts #42 / #45 (in-stock ratio surfacing in top-K), #46 (size_range AI-tagger reliability gap), #49 (broader category-coverage gap), and #51 (pre-existing-rules discrepancy that produced the kurta regression). Each of those op debts is one symptom of a single underlying property: **Stages 2-5 don't weight tag-set overlap with extracted query attributes as a ranking signal.**

Three potential fixes carry into 3.1.8 planning:
- **(a) Stage 3 rerank tag-overlap signal:** add an explicit weight to Stage 3's rerank for "how many of the query's extracted attributes does this candidate's APPROVED tag set satisfy?"
- **(b) Stage 5 quota policy:** add a per-fixture-axis-coverage quota so the top-K must include at least N cards satisfying the per-axis filter (when feasible from the candidate pool).
- **(c) Proportional coverage discipline:** treat secondary-axis coverage as a *prerequisite* for category coverage expansion. mech.4's lesson: don't add 200 kurta rows without also adding fit tags to those 200 products.

None of these is in 3.1.7's scope. Carry to 3.1.8 planning round as a fork between "ship the flip without addressing the ranking gap (R3.1 = 0.2917 acceptable)" vs "fix the ranking gap before the flip (R3.1 = 0.3333+ target)." That decision belongs at 3.1.8 planning round, not in this close commit.

#### 3.1.8 prerequisites (op debts surfaced during 3.1.7, cross-referenced)

- **#15** — variant-loading wired (closed at mech.2 via Stage 5.5 `loadAndAttachVariants`).
- **#43** — `gender=female` axis at 0% APPROVED. Will block `fashion-festive-kurta-women` fixture indefinitely. Likely needs same rule-engine treatment as mech.4 applied for shorts (e.g., `title_contains "women"` → `gender=female` rule). 3.1.8 planning fork.
- **#42 / #45** — Stage 6 binary `available` vs Stage 3 weighting; dev shop's 2.5% in-stock ratio is a "real merchant pathology" question (29 of 1,169 ACTIVE products have any `availableForSale=true` variant). 3.1.8 planning fork.
- **#46** — `size_range` AI-tagger reliability (75% sub-0.8 confidence; only 3 distinct products with APPROVED rows). Binding constraint for `fashion-summer-shorts-size-m` fixture's continued FAIL=0 post-mech.4 (Stage 1 unblocked from 0 → 53 candidates, but top-6 lacks size_range coverage).
- **#49** — broader category-coverage gap (~74% of catalog still untagged on category axis post-mech.4 — 303 of 1,168 ACTIVE products carry APPROVED category, with most coverage gain from the retroactive kurta rule). Closing the rest requires more rule-engine seeds, AI-tagger re-runs, or both.
- **#50** — multi-tenant verification (non-fashion store modes). Phase 5 territory.
- **#51** — dev shop's TaggingRule table diverges from `SEED_RULES.FASHION` (pre-existing `Kurta → category=kurta`, `Jeans → category=jeans`, `Linen mention → sub_category=linen`, `Cotton mention → sub_category=cotton` rows don't match the codebase definition). Decision needed at 3.1.8 planning: which is canonical?

3.1.8's scope at planning round will be: the v1→v2 flip (per planning Section 6 line 156), plus whatever subset of #43, #45, #46, #49, #51 the planning round decides is prerequisite, plus a decision on the (a)/(b)/(c) fork for the ranking-vs-tag-completeness gap. Op debt #50 is explicitly Phase 5 territory.

#### Closing meta

3.1.7 ships entirely against the dev shop + eval harness + unit tests; no user-visible storefront change (the flip is deferred to 3.1.8 per the planning round's locked architecture). The compensation is that 3.1.8's flip ships against a non-degenerate eval baseline (0.2917) with op debts #11 and #15 already closed, and with concrete architectural evidence (the ranking-vs-tag-completeness finding) available for the 3.1.8 planning round to weigh.

Total chain: 4 mechs + 1 conditional skip (saree, per op debt #48) + close (this commit). Four mech.N.5 verification commits (mech.1.5, mech.2.5, mech.3.5, mech.4.5). One tool-extension commit (mech.3a) spawned mid-chain when mech.3's decision gate fired and the bulk-approve script needed a `--min-confidence` flag. Slightly larger than 3.1.6's "3 mechs + close" split as the planning round anticipated; the variant filter relocation (mech.1) and the eval re-anchoring (mech.5) each carried their own mech in addition to the chain's main work.

**Sub-bundle 3.1.7 closes here.** Next: Sub-bundle 3.1.8 planning round (v1→v2 flip + 3.1.8-prerequisite subset from op debts #43/#45/#46/#49/#51 + ranking-vs-tag-completeness fork decision). After 3.1.8: Sub-bundle 3.2 planning round (order ingest + sales velocity + `AttributionEvent`).

### Sub-bundle 3.1.8 planning round close: Option α + #54 denominator correction locked

3.1.8 planning round closes here. Three threads completed: Thread 1 (flip-site re-verification, `7a565aa`) + Thread 3 (ranking-gap fork resolution, `d787df9`) + Thread 2 (prerequisite triage, `6564a0b`). All 32 investigation artifacts in `.pr-3-1-8-planning-artifacts/`. Locked architecture in `docs/planning/3-1-8-flip-and-measurement-correction.md`.

**Locked: Option α (flip first, ranking-gap fix in 3.2) + bundle #54 (relaxedMatchAtK denominator correction) into 3.1.8.**

The 3.1.7 close framed 3.1.8 as "flip + prerequisite-subset + ranking-gap fork." Thread 3 inverted three pre-investigation predictions: fix (a) produces NULL eval impact (Stage 5 ignores Stage 3 rerankBoosts in selection); the bottleneck is Stage 2 narrowing not Stage 3 rerank; Option α (flip first) is the right ordering because real-merchant traffic is the best test data for d.2-strict's empty-Stage-1 risk. Thread 2 triaged 9 op debts: F=0, B=1 (#54), N=8 (#43, #45, #46, #49, #51, #52, #53, #55), P=0. The B verdict on #54 is the load-bearing finding — switching to K-based normalization corrects a measurement contract that retroactively explains the 3.1.7 mech.4 kurta regression as an artifact rather than a real eval drop.

**Mech chain for 3.1.8:** mech.1 (denominator switch) → mech.2 (v1→v2 flip) → mech.2.5 (verification) → mech.3 (legacy v1 deletion) → mech.4 (close + HANDOFF amendment). Total: ~4-5 commits.

**R3 ladder re-anchored:** R3.0 = 0.2778 (post-denominator-switch baseline; K-based normalization). R3.1 ≥ 0.2778 (3.1.8 flip target — no regression). R3.2 ≥ 0.50 (Phase 5 multi-mode target — unchanged). Historical R3 values (0.0833, 0.1667, 0.3333, 0.2917) are non-comparable to post-3.1.8 due to the denominator semantics change.

**Architectural finding retroactive reframing:** Under K-based normalization, mech.4 from 3.1.7 was eval-invisible rather than a regression. The Stages 2-5 ranking architectural finding (Stages 2-5 don't favor tag-complete candidates) still holds and motivates 3.2's d.2-strict; the measurement artifact that surfaced it as a regression dissolves.

**3.2 prerequisites (op debts surfaced or carried during 3.1.8):**

- **#43** — gender=female axis at 0% APPROVED. Carry to 3.2 alongside #46 + #49.
- **#45** — in-stock ratio question. Carry to 3.2 or Phase 5+.
- **#46** — size_range AI-tagger reliability. Carry to 3.2 alongside d.2-strict.
- **#49** — broader category-coverage gap. Carry to 3.2 (depends on #51 direction).
- **#51** — dev shop SEED_RULES divergence. Direction NAMED at this close: option (c) — "SEED_RULES is the starter template, not canonical for any deployed shop." Implementation in 3.2.
- **#52** — Stage 5 ignores rerankBoosts. Carry to 3.2 or Phase 5+.
- **#53** — Stage 2 candidatePool dilution. Carry to 3.2 as the d.2-strict primary fix site.
- **#54** — relaxedMatchAtK denominator semantics. **CLOSED in 3.1.8-mech.1** (this is the bundle work locked at planning round close).
- **#55** — deferred architectural finding. Carry to 3.2 by definition.

**3.2 cluster framing** (per Thread 2 § 5):
- **Cluster 1 (ranking-architecture):** #52, #53, #55, d.2-strict implementation.
- **Cluster 2 (catalog-data):** #46, #49, #51.
- **Cluster 3 (deferred):** #43, #45.

Two clusters fit within a single 3.2 planning round; three clusters likely splits across 3.2 and 3.3.

**Investigation artifacts:** 32 total (Thread 1 = 01-08; Thread 3 = 09-20 plus 2 throwaway probes; Thread 2 = 21-32 plus 2 empirical-state probes). Plus 3 thread synthesis artifacts (08, 20, 32) that are the load-bearing decision inputs.

**Sub-bundle 3.1.8 planning round closes here.** Next: 3.1.8 mech chain begins with mech.1 (relaxedMatchAtK denominator switch). After 3.1.8 closes: 3.2 planning round (ranking-architecture cluster + catalog-data cluster).
